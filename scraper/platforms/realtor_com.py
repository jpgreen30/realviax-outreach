...
class RealtorComScraper:
    ...
    def scrape_listing(self, url: str) -> Optional[Dict[str, Any]]:
        try:
            ...
            photos = [p for p in photos if 'realtor' in p.lower()][:20]
            ...
            logger.info(f"Scraped Realtor.com listing: {address_data.get('address')}")
            return result
            
        except Exception as e:
            logger.error(f"Realtor.com scrape failed for {url}: {e}")
            return None
